...
//         if (characterMovement.playerIsCustomizing == true)
//         {
//           RotateCharacterForCustomization();
//         }
//         else if (characterMovement.playerOnBike == true)
//         {
//         //RideBike
//         RideBike();
//         }
...
//     public void RotateCharacterForCustomization()
//     {
...
//           upLeftAnim
//       };
//       currentAnimationDirection = animValues[(currentAnimValue / animDirectionAdjustorInt) % animValues.Length];
//       Debug.Log("Current Animation Value: " + currentAnimValue);

//       // Move to the next animation value in the next cycle
...
public class CharacterAnimation : MonoBehaviour
{
    // Character animation and color properties
    public List<GameObject> characterSpriteList = new List<GameObject>();
    public List<Color> initialChrctrColorList = new List<Color>();
    public float initialAnimationSpeed;
    public float animationSpeed = 0.09f;

    // Animation direction and states
    public int animDirectionAdjustorInt = 26;
    public int rightDownAnim, leftDownAnim, rightAnim, leftAnim, upRightAnim, upLeftAnim;
    public int ladderAnimDirectionIndex;
    public int idle = 0, walk = 1, run = 5, sit = 9, climb = 11, rideBike = 15;
    ...
    // Sprite renderers and dictionaries
    private Dictionary<string, Sprite[]> spriteDictionary;
    private Dictionary<string, SpriteRenderer> spriteRendererDictionary;
    public int bodyTypeNumber, bodyTypeIndexMultiplier = 156;

    // Colors
    public Color currentSkinColor, currentHairColor, currentShirtColor, currentPantsColor, currentShoeColor, currentJakettoColor;
    public Color currentBikeColor, bikeInitialColor;

    // Coroutine and component references
    private Coroutine allowTimeForSpaceBarCoro, characterCustomizationIdleCoro;
    ...
    {
        spriteRendererDictionary = new Dictionary<string, SpriteRenderer>();
        foreach (var sr in GetComponentsInChildren<SpriteRenderer>())
        {
            spriteRendererDictionary[sr.gameObject.name] = sr;
        }
    ...
    void LoadSprites()
    {
        ...
        {
            {"head", Resources.LoadAll<Sprite>("head")},
            {"eyes", Resources.LoadAll<Sprite>("eyes")},
            {"throat", Resources.LoadAll<Sprite>("throat")},
            {"collar", Resources.LoadAll<Sprite>("collar")},
            {"torso", Resources.LoadAll<Sprite>("torso")},
            {"waist", Resources.LoadAll<Sprite>("waist")},
            {"waistShorts", Resources.LoadAll<Sprite>("waistShorts")},
            {"kneesShins", Resources.LoadAll<Sprite>("kneesShins")},
            {"ankles", Resources.LoadAll<Sprite>("ankles")},
            {"feet", Resources.LoadAll<Sprite>("feet")},
            {"jaketto", Resources.LoadAll<Sprite>("jaketto")},
            {"dress", Resources.LoadAll<Sprite>("dress")},
            {"longSleeve", Resources.LoadAll<Sprite>("longSleeve")},
            {"hands", Resources.LoadAll<Sprite>("hands")},
            {"shortSleeve", Resources.LoadAll<Sprite>("shortSleeve")},
            {"mohawk5Top", Resources.LoadAll<Sprite>("mohawk5Top")},
            {"mohawk5Bottom", Resources.LoadAll<Sprite>("mohawk5Bottom")},
            {"hair0Top", Resources.LoadAll<Sprite>("hair0Top")},
            {"hair0Bottom", Resources.LoadAll<Sprite>("hair0Bottom")},
            {"hair1Top", Resources.LoadAll<Sprite>("hair1Top")},
            {"hair7Top", Resources.LoadAll<Sprite>("hair7Top")},
            {"hair8Top", Resources.LoadAll<Sprite>("hair8Top")},
            {"hair1Bottom", Resources.LoadAll<Sprite>("hair1Bottom")},
            {"hair2Bottom", Resources.LoadAll<Sprite>("hair2Bottom")},
            {"hair3Bottom", Resources.LoadAll<Sprite>("hair3Bottom")},
            {"hair4Bottom", Resources.LoadAll<Sprite>("hair4Bottom")},
            {"hair6Bottom", Resources.LoadAll<Sprite>("hair6Bottom")},
            {"hair7Bottom", Resources.LoadAll<Sprite>("hair7Bottom")},
            {"hair8Bottom", Resources.LoadAll<Sprite>("hair8Bottom")},
            {"hairFringe1", Resources.LoadAll<Sprite>("hairFringe1")},
            {"hairFringe2", Resources.LoadAll<Sprite>("hairFringe2")},
            {"bike", Resources.LoadAll<Sprite>("bike")}
        };
    }

    void AssignInitialSprites()
    {
        foreach (var entry in spriteDictionary)
        {
            if (spriteRendererDictionary.TryGetValue(entry.Key, out var renderer))
            {
                renderer.sprite = entry.Value[bodyTypeNumber * bodyTypeIndexMultiplier];
            }
        }
    }
...
public void Animate(int movementStartIndex, int movementFrameCount, int animationDirection, int bodyTypeNumber)
    ...
    public void HandleGettingOffBikeAnim()
    {
       if(characterMovement.spaceBarDeactivated == false)
        {
            if ((Input.GetKey(KeyCode.Space) || 
